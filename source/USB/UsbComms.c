...
#define R_FAILED_LOG_RETURN(x) do { if (R_FAILED(x)) { LOG("%x @ %u", x, __LINE__); return rc; } } while (0)
...
Result UsbCommsInitialize(struct usb_device_descriptor* device_descriptor, u32 num_interfaces, const UsbInterfaceDesc* infos)
{
	...
	if (g_usbCommsInitialized) {
		...
		if (R_FAILED(rc)) LOGs("usbDsInitialize");
...
static Result _usbCommsInterfaceInit5x(u32 intf_ind, const UsbInterfaceDesc* info)
{
	...
	if (info->ConfigDescriptors.Addr && info->ConfigDescriptors.Len)
	{
		rc = usbDsInterface_AppendConfigurationData(interface->interface, UsbDeviceSpeed_Full, info->ConfigDescriptors.Addr, info->ConfigDescriptors.Len);
		R_FAILED_LOG_RETURN(rc);
		rc = usbDsInterface_AppendConfigurationData(interface->interface, UsbDeviceSpeed_High, info->ConfigDescriptors.Addr, info->ConfigDescriptors.Len);
		R_FAILED_LOG_RETURN(rc);
		rc = usbDsInterface_AppendConfigurationData(interface->interface, UsbDeviceSpeed_Super, info->ConfigDescriptors.Addr, info->ConfigDescriptors.Len);
		R_FAILED_LOG_RETURN(rc);
	}
...
UsbDsInterface* UsbCommsGetInterface(u8 index)
{
	if (g_usbCommsInitialized && index < TOTAL_INTERFACES)
		return g_usbCommsInterfaces[index].interface;
	return NULL;
}
